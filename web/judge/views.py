...
from invitation import accept_invitation
  ...
  if request.method == 'POST':
    ...
    if update_github_form.has_changed():
      if update_github_form.is_valid():
        ...
        n_undone_forms -= 1
        inv_stats = accept_invitation(update_github_form.data['github_account'], update_github_form.data['github_repository'])
        if(inv_stats == 0):
          messages.success(request,
                           'GitHub settings successfully updated. The collaborator invitation has been accepted.')
        elif(inv_stats == 1):
          messages.error(request, 'Invitation list fetch error. Please contact administrator.')
        elif(inv_stats == 2):
          messages.error(request, 'Oops! It seems that you haven\'t sent an collaborator invitation to the OOP Judge GitHub account or you entered the wrong account/repository name.')
          messages.error(request, 'Please check your account/repository name and try again after you add the OOP Judge GitHub account as a collaborator of your repository.')
        elif(inv_stats == 3):
          messages.error(request, 'Accept invitation failed. Please contact administrator.')
        elif(inv_stats == 4):
          messages.error(request, 'Oops! It seems that you haven\'t sent an collaborator invitation to the OOP Judge GitHub account or you entered the wrong account/repository name.')
          messages.error(request, 'Please check your account/repository name and try again after you add the OOP Judge GitHub account as a collaborator of your repository.')
        else:
          messages.error(request, 'Unknown error. Please contact administrator.')
